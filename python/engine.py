...
def clean_2b(raw_bytes):
    ...
    df["_v"] = pd.to_numeric(df["Invoice Value(\u20b9)"], errors="coerce")
    df = (df.sort_values(["GSTIN of supplier", "Invoice Date", "_v"],
...
def clean_tally(raw_bytes):
    ...
    df["_v"] = pd.to_numeric(df["Invoice Value(\u20b9)"], errors="coerce")
    df = (df.sort_values(["_blank", "GSTIN of supplier", "Invoice Date", "_v"],
                         ascending=[False, True, True, False], kind="stable")
            .drop(columns=["_blank", "_v"]).reset_index(drop=True))
...
def merge_2b(latest_df, cf_bytes):
    ...
    combined["_v"] = pd.to_numeric(combined["Invoice Value(\u20b9)"], errors="coerce")
    combined = (combined.sort_values(["GSTIN of supplier", "Invoice Date", "_v"],
                                     ascending=[True, True, False], kind="stable")
                .drop(columns=["_v"]).reset_index(drop=True))
...
def merge_tally(latest_df, cf_bytes):
    ...
    combined["_v"] = pd.to_numeric(combined["Invoice Value(\u20b9)"], errors="coerce")
    combined = (combined.sort_values(["_blank", "GSTIN of supplier", "Invoice Date", "_v"],
                                     ascending=[False, True, True, False], kind="stable")
                .drop(columns=["_blank", "_v"]).reset_index(drop=True))
    return combined, {"merged": True, "cf_rows": len(cf), "latest_rows": len(latest_df), "rows": len(combined)}
...
def _within(a, b, tol=AMT_TOL):
    av = float(a) if pd.notna(a) else 0.0
    bv = float(b) if pd.notna(b) else 0.0
...
def _amounts_match(b, t):
    if not _within(b["Invoice Value(\u20b9)"], t["Invoice Value(\u20b9)"]):
        return False
    for c in ("Central Tax(\u20b9)", "State/UT Tax(\u20b9)", "Integrated Tax(\u20b9)"):
        if not _within(b.get(c, 0), t.get(c, 0)):
            return False
    return True
...
def _remark(b, t, where):
    parts = []
    dv = abs(float(b["Invoice Value(\u20b9)"] or 0) - float(t["Invoice Value(\u20b9)"] or 0))
    if dv > AMT_TOL:
        parts.append(f"Invoice Value differs by \u20b9{dv:.2f}")
    for lab, col in (("CGST", "Central Tax(\u20b9)"), ("SGST", "State/UT Tax(\u20b9)"), ("IGST", "Integrated Tax(\u20b9)")):
        d = abs(float(b.get(col, 0) or 0) - float(t.get(col, 0) or 0))
        if d > AMT_TOL:
            parts.append(f"{lab} differs by \u20b9{d:.2f}")
